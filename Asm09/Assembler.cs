...
class Assembler : BigEndianAssembler
{
    ...
    {
        isDirect = version switch
        {
            >= 2 => value => IsLowByte(value) || IsZeroPage(value),
            _ => IsLowByte
        };
    }

    private static bool IsZeroPage(Address value)
    {
        return value.Type == AddressType.ZeroPage;
    }

    private static bool IsLowByte(Address value)
    {
        return value.Part == AddressPart.LowByte;
    }

    private readonly Func<Address, bool> isDirect;

    //private bool IsLowByte(Address value)
    //{
    //    return value.Part == AddressPart.LowByte;
    //}
